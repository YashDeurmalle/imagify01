...
const BuyCredit = () => {
  ...
  const {user, backednUrl, loadCreditsData, token, setShowLogin} = useContext(Appcontext);
  ...
  //     const {data} =  await axios.post(backednUrl + '/api/user/pay-razor', {planId}, {headers:{token}})
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index) =>(
          <div className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-600 hover:scale-105 transition-all duration-500' key={index}>
            ...
            <p className='mt-6'><span className='text-3xl font-medium'></span>${item.price} / {item.credits} credits</p>
